...
class m_goods extends base_m {
	...
	public function create($data) {
        ...
        $data ['in_price'] = $data ['in_price'] ? $data ['in_price'] : 1;
        $data ['weight'] = $data ['weight'] ? $data ['weight'] : 1;
        $data ['in_num'] = $data ['in_num'] ? $data ['in_num'] : 1;
        $data ['unit'] = $data ['unit'] ? $data ['unit'] : "个";
        $data ['promote_price'] = $data ['promote_price'] ? $data ['promote_price'] : 0;

        $data ['promote_start_date'] = $data ['promote_start_date'] ? $data ['promote_start_date'] : date ( "Y-m-d", time () );
        $data ['promote_end_date'] = $data ['promote_end_date'] ? $data ['promote_end_date'] : date ( "Y-m-d", time () );
        $data ['lastinymd'] = date ( "Y-m-d", time () );
        $data ['lastindateline'] = 10;
        $data ['salesamount'] = 0;
        $data ['countamount'] = 0;
        
        $data ['goods_desc'] = $data ['goods_desc'] ? $data ['goods_desc'] : "空";
		...
		$this->set ( "lastindateline", $data ['lastindateline'] );
		$this->set ( "countamount", $data ['countamount'] );
		$this->set ( "salesamount", $data ['salesamount'] );
	...
	public function getSalePrice($goods_sn) {
		...
		if ($goods ['ispromote'] == 1 and $ymd > $goods ['promote_start_date'] and $ymd < $goods ['promote_end_date']) {
			...
			$data ['promote_price'] = $goods ['out_price'];
		}
		return $data;
	}
	 ...
	 * @return Array
	 */
	public function getGoodsUnit($goods_id) {
		$goods = $this->get ( "goods_id='{$goods_id}'" );
		if (! $goods)
			return false;
